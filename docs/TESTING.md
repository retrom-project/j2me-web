...
生命周期夹具在截图检查后通过左软键结束写入，并发出 `LIFECYCLE_SAVE_READY`。测试等待 RMS 稳定后才暂停、导出和核对恢复值，避免把 JVM 暂停在写入中途后等待一个无法完成的写入；前面的暂停/恢复断言仍覆盖持续运行的 Java 线程。
...
The rendering workload also measures full-screen ARGB/RGBA conversion and 32 overlapping translucent 64×64 sprites, separately from presentation pacing. Bulk budgets are 2 ms for each full-screen blit/conversion and 8 ms for the sprite workload. Alpha regression checks the actual native binding, negative strides, byte order, round trips and rejection without mutation. The core build runs 1,000 C pixel cases against an independent floating-point reference and the Java fallback/clipping cases before compiling Wasm; the C test can additionally run with AddressSanitizer and UndefinedBehaviorSanitizer.

`npm run test:presentation` uses the owned PresentationMidlet. Twelve spaced game paints must yield 10–13 core submissions (allowing a coalesced boundary), instead of the previous 48. An idle interval must produce zero submissions and zero display copies. It verifies exact logical screenshot pixels and display pixels through all scaling modes while paused, and a standard gamepad right soft key after the screen becomes idle. It does not increase the game frame limit or require unchanged screens to report 60 FPS.
