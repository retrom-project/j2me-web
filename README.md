...
当前版本为 `0.3.4`。构建输入现由 `retrom-project` organization 管理；运行时排除上游附带的游戏 JAR，并提供显式 PFB 核心候选构建接口。当前 checkpoint 仍是 RMS 文件树，通过 Retrom 的 `GAME_SAVE` 语义接入，需要在游戏内保存和读档，不提供即时执行状态恢复。
